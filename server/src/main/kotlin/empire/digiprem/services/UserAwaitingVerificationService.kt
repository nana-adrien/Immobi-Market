...
class UserAwaitingVerificationService(
    ...
    private val userDetailService: IUserDetailService? = null
    ...
    fun getIdentifyUser(identity: String): Users {
        val userAwaitingVerification =
            userAwaitingVerificationRepository.getUserAwaitingVerificationByIdentifier(identity).getOrNull()
    ...
    private fun createUser(userAwaitingVerification: UserAwaitingVerification): Users {
        val countryCode = userAwaitingVerification.countryCode + ""
        val userPhoneNumber = UserPhoneNumber(
            phoneNumber = userAwaitingVerification.identifier.substring(countryCode.length),
            countryCode = userAwaitingVerification.countryCode,
        )
        val user = Users(
            email = if (userAwaitingVerification.isEmail) userAwaitingVerification.identifier else "",
            userPassword = passwordEncoder.encode(userAwaitingVerification.password),
            userPhoneNumber = if (userAwaitingVerification.isEmail) null else userAccountService!!.createPhoneNumber(
                userPhoneNumber
            ),
        )
        return user
