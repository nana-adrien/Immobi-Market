...
class WebSocketAuthenticationInterceptor(
    ...
    override fun preSend(message: Message<*>, channel: MessageChannel): Message<*>? {
       ...
       // WebSocketAuthenticationInterceptor.log.info("Headers: {}", accessor)
        ...
        if (StompCommand.CONNECT.equals(accessor.getCommand())) {
             ...
             */
            val authorizationHeader: String = checkNotNull(accessor.getFirstNativeHeader("Authorization"))
            /* On enlève le préfixe "Bearer " du token pour obtenir seulement le token JWT
             */
            val token =
                authorizationHeader.substring(7) /* Le "7" correspond à la longueur du mot "Bearer " pour couper le préfixe*/
